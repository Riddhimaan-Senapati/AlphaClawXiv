...
not recognize the expired/revoked token). If the token exists but was created
without `--bypass-2fa`, the publish fails with EOTP instead:
...
Refresh the token before each release, logged in as the npm account that owns
the package (`npm whoami` must return that user), with `--bypass-2fa` so CI can
publish without an interactive OTP:

```powershell
npm token create --name "alphaclawxiv-publish" --packages alphaclawxiv --packages-and-scopes-permission read-write --bypass-2fa
gh secret set NPM_TOKEN --body "<token>"
```

`npm token create` requires `--name` in npm 11; omitting it fails with
`Token name is required`. Do not pass `--read-only` (it blocks publishing).
Token names must be unique: if a token with the name already exists, npm fails
with `Duplicate token names are not allowed`. Use a unique name per release
(e.g. `alphaclawxiv-publish-20260906`), or revoke the old token first with
`npm token list` then `npm token revoke <id>`.

The ClawHub `CLAWHUB_TOKEN` is stored in the clawhub CLI at `%APPDATA%\clawhub`
and is validated with `clawhub whoami`.
